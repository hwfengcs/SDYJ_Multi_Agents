...
1. 先识别摘要中反复出现、对回答研究问题最关键的概念。
2. 合并同义主题，避免把同一问题拆成多个重复主题。
3. 每个要点都要能映射回摘要中的事实、数据、案例或明确判断。
4. 如果摘要证据不足，不要编造主题；用更保守的表述。
</internal_reasoning_checklist>
  ...
  "themes": [
    {
      "name": "可靠性评估",
      "key_points": [
        "评估应同时覆盖答案质量、检索质量与工具失败恢复，避免只用单一准确率指标判断系统可靠性。",
        "可回放 trace 能把失败定位到计划、检索、合成或验证环节，为回归测试提供稳定依据。"
      ]
    }
  ]
}
