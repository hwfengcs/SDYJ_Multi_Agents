...
项目已从基础 MVP 升级到更适合作为 AI Agent 算法工程师实习作品集的版本：

- 检索结果会规范化为 `E1/E2/...` 证据项，保留 source、query、URL、domain、发布时间和相关性分数。
- 报告生成阶段会把关键发现绑定到 evidence ID，并在参考资料中展示可追踪来源。
- 每次研究或评测都会写入 JSON trace，记录节点、LLM 调用、工具调用、latency、错误和报告指标。
- CLI 支持 `inspect-run`、`list-scenarios` 和 `eval`，可运行离线评测与真实 DeepSeek live 评测。
- 内置困难评测场景覆盖 RAG、网页检索、MCP 工具、人机审批、故障恢复、成本/延迟和 ablation study。
